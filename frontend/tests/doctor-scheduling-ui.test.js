...
check('responsive layout has desktop tablet and mobile breakpoints', /@media\s*\(\s*max-width\s*:\s*900px\s*\)/.test(scheduleCss) && /@media\s*\(\s*max-width\s*:\s*600px\s*\)/.test(scheduleCss));
...
check('professional profile displays verified credentials read-only', /<input\b(?=[^>]*\bid="verifiedSpecialty")(?=[^>]*\breadonly\b)[^>]*>/.test(profileHtml) && /<input\b(?=[^>]*\bid="verifiedLicense")(?=[^>]*\breadonly\b)[^>]*>/.test(profileHtml));
