-- 11_clinics_faraa_nudge_cleanup.sql
-- Same gap-fill/defect repair as 09_clinics_nablus_cleanup.sql, extended to the
-- wider Nablus governorate bbox (32.15-32.29 lat / 35.15-35.35 lng) to cover
-- villages outside the city-center bbox (Huwara, Awarta, Beit Furik, Sabastia,
-- Deir Sharaf, Far'a camp, Jit, Burin, etc). Same rules: never overwrite a real
-- value, '' not 'N/A', matched on exact (latitude, longitude).
...
-- osm_node:431901338 -- https://www.openstreetmap.org/node/431901338
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'clinic' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'منزل الدكتور جميل' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'منزل الدكتور جميل' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN '' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN '' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN NULL ELSE phone END,
  updated_at = NOW()
...
-- osm_node:431905070 -- https://www.openstreetmap.org/node/431905070
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'clinic' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'منزل الدكتور منير شريف' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'منزل الدكتور منير شريف' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN '' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN '' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN NULL ELSE phone END,
  updated_at = NOW()
...
-- osm_node:431919386 -- https://www.openstreetmap.org/node/431919386
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'hospital' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'مركز صحي' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'مركز صحي' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN '' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN '' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN NULL ELSE phone END,
  updated_at = NOW()
...
-- osm_node:431938150 -- https://www.openstreetmap.org/node/431938150
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'pharmacy' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'صيدلية بورين' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'صيدلية بورين' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN '' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN '' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN NULL ELSE phone END,
  updated_at = NOW()
...
-- osm_node:431960610 -- https://www.openstreetmap.org/node/431960610
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'clinic' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'عيادة الدكتور عبد الناصر' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'عيادة الدكتور عبد الناصر' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN '' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN '' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN NULL ELSE phone END,
  updated_at = NOW()
...
-- osm_node:431986017 -- https://www.openstreetmap.org/node/431986017
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'pharmacy' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'صيدلية المنار' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'صيدلية المنار' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN '' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN '' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN NULL ELSE phone END,
  updated_at = NOW()
...
-- osm_node:432005128 -- https://www.openstreetmap.org/node/432005128
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'pharmacy' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'صيدلية عورتا' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'صيدلية عورتا' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN '' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN '' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN NULL ELSE phone END,
  updated_at = NOW()
...
-- osm_node:432046159 -- https://www.openstreetmap.org/node/432046159
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'pharmacy' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'صيدلية الترياق' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'صيدلية الترياق' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN '' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN '' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN NULL ELSE phone END,
  updated_at = NOW()
...
-- osm_node:432065174 -- https://www.openstreetmap.org/node/432065174
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'clinic' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'عيادة سبسطية' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'عيادة سبسطية' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN '' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN '' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN NULL ELSE phone END,
  updated_at = NOW()
...
-- osm_node:432077445 -- https://www.openstreetmap.org/node/432077445
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'clinic' ELSE type END,
...
-- osm_node:432081304 -- https://www.openstreetmap.org/node/432081304
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'clinic' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'عيادة بيت فوريك' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'عيادة بيت فوريك' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN '' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN '' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN NULL ELSE phone END,
  updated_at = NOW()
...
-- osm_node:432086911 -- https://www.openstreetmap.org/node/432086911
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'pharmacy' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'صيدلية الهلال' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'صيدلية الهلال' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN '' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN '' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN NULL ELSE phone END,
  updated_at = NOW()
...
-- osm_node:432098691 -- https://www.openstreetmap.org/node/432098691
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'clinic' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'عيادة طبية' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'عيادة طبية' ELSE name_en END,
...
-- osm_node:432101121 -- https://www.openstreetmap.org/node/432101121
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'hospital' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'مستوصف قريه سالم' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'مستوصف قريه سالم' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN '' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN '' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN NULL ELSE phone END,
  updated_at = NOW()
...
-- osm_node:432114106 -- https://www.openstreetmap.org/node/432114106
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'clinic' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'مركز ابن سينا ( الاغاثه )' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'مركز ابن سينا ( الاغاثه )' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN '' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN '' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN NULL ELSE phone END,
  updated_at = NOW()
...
-- osm_node:432121834 -- https://www.openstreetmap.org/node/432121834
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'clinic' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'العيادة الصحية' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'العيادة الصحية' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN '' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN '' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN NULL ELSE phone END,
  updated_at = NOW()
...
-- osm_node:432124248 -- https://www.openstreetmap.org/node/432124248
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'clinic' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'عيادة الدكتور سالم' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'عيادة الدكتور سالم' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN '' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN '' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN NULL ELSE phone END,
  updated_at = NOW()
...
-- osm_node:432129443 -- https://www.openstreetmap.org/node/432129443
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'pharmacy' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'صيدلية حوارة' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'صيدلية حوارة' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN '' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN '' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN NULL ELSE phone END,
  updated_at = NOW()
...
-- osm_node:432135272 -- https://www.openstreetmap.org/node/432135272
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'pharmacy' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'صيدلية عوريف' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'صيدلية عوريف' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN '' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN '' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN NULL ELSE phone END,
  updated_at = NOW()
...
-- osm_node:432166016 -- https://www.openstreetmap.org/node/432166016
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'clinic' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'مركز سبسطية الصحي' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'مركز سبسطية الصحي' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN '' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN '' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN NULL ELSE phone END,
  updated_at = NOW()
...
-- osm_node:432186034 -- https://www.openstreetmap.org/node/432186034
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'pharmacy' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'صيدلية الاسره' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'صيدلية الاسره' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN '' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN '' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN NULL ELSE phone END,
  updated_at = NOW()
...
-- osm_node:432319825 -- https://www.openstreetmap.org/node/432319825
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'clinic' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'Shaher Mansour' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'Shaher Mansour' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN '' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN '' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN NULL ELSE phone END,
  updated_at = NOW()
WHERE latitude = 32.191869 AND longitude = 35.1563408
  AND (
    type IS NULL
    OR name_ar = name_en
    OR address_ar IN ('N/A', '')
...
-- osm_node:432329073 -- https://www.openstreetmap.org/node/432329073
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'clinic' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'عيادة امومة' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'عيادة امومة' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN '' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN '' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN NULL ELSE phone END,
  updated_at = NOW()
...
-- osm_node:4227034305 -- https://www.openstreetmap.org/node/4227034305
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'clinic' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'Al-Fara''a Health Clinic' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'Al-Fara''a Health Clinic' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN 'Tubas - Al Far''a Camp' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN 'Tubas - Al Far''a Camp' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN NULL ELSE phone END,
  updated_at = NOW()
WHERE latitude = 32.29219 AND longitude = 35.345588
  AND (
    type IS NULL
    OR name_ar = name_en
    OR address_ar IN ('N/A', '')
...
-- osm_node:4464809795 -- https://www.openstreetmap.org/node/4464809795
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'clinic' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'المستودعات المركزية -وزارة الصحة' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'المستودعات المركزية -وزارة الصحة' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN '' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN '' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN NULL ELSE phone END,
  updated_at = NOW()
WHERE latitude = 32.208827 AND longitude = 35.3165777
  AND (
    type IS NULL
    OR name_ar = name_en
    OR address_ar IN ('N/A', '')
...
-- osm_node:5001105721 -- https://www.openstreetmap.org/node/5001105721
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'clinic' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'منزل الدكتور محمد أحمد صالح' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'Dr Mohammad Saleh''s house' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN 'main street' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN 'main street' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN NULL ELSE phone END,
  updated_at = NOW()
WHERE latitude = 32.1792146 AND longitude = 35.215415
  AND (
    type IS NULL
    OR name_ar = name_en
    OR address_ar IN ('N/A', '')
...
-- osm_node:5283315121 -- https://www.openstreetmap.org/node/5283315121
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'clinic' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'Dr.atef clinic' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'Dr.atef clinic' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN '' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN '' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN NULL ELSE phone END,
  updated_at = NOW()
...
-- osm_node:5315485923 -- https://www.openstreetmap.org/node/5315485923
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'pharmacy' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'rawan pharmacy' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'rawan pharmacy' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN '' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN '' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN '092578287' ELSE phone END,
  updated_at = NOW()
WHERE latitude = 32.2911133 AND longitude = 35.3453395
  AND (
    ...
    OR address_ar IN ('N/A', '')
    OR address_en IN ('N/A', '')
    OR (phone IS NULL AND '092578287' IS NOT NULL)
  );

-- osm_node:5318288524 -- https://www.openstreetmap.org/node/5318288524
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'pharmacy' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'Rawan pharmacy' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'Rawan pharmacy' ELSE name_en END,
...
-- osm_node:5458133521 -- https://www.openstreetmap.org/node/5458133521
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'clinic' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'Ali Khalifa''s Clinic' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'Ali Khalifa''s Clinic' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN '' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN '' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN NULL ELSE phone END,
  updated_at = NOW()
...
-- osm_node:5645800821 -- https://www.openstreetmap.org/node/5645800821
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'pharmacy' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'صيدلية نور الخافظ' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'صيدلية نور الخافظ' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN 'Odala Street' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN 'Odala Street' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN NULL ELSE phone END,
  updated_at = NOW()
WHERE latitude = 32.1570809 AND longitude = 35.2765981
  AND (
    type IS NULL
    OR name_ar = name_en
    OR address_ar IN ('N/A', '')
...
-- osm_node:5645800921 -- https://www.openstreetmap.org/node/5645800921
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'pharmacy' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'صيدلية النصر' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'صيدلية النصر' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN 'شارع عورتا' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN 'شارع عورتا' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN NULL ELSE phone END,
  updated_at = NOW()
WHERE latitude = 32.1619637 AND longitude = 35.2887914
  AND (
    type IS NULL
    OR name_ar = name_en
    OR address_ar IN ('N/A', '')
...
-- osm_node:5667327122 -- https://www.openstreetmap.org/node/5667327122
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'pharmacy' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'صيدلية هادي' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'صيدلية هادي' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN '‭‬092531383, Bait Imrin-Yasid' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN '‭‬092531383, Bait Imrin-Yasid' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN NULL ELSE phone END,
  updated_at = NOW()
...
-- osm_node:6013018885 -- https://www.openstreetmap.org/node/6013018885
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'pharmacy' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'صيدلية لانا كير' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'صيدلية لانا كير' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN '' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN '' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN NULL ELSE phone END,
  updated_at = NOW()
WHERE latitude = 32.225369 AND longitude = 35.2088237
  AND (
    type IS NULL
    OR name_ar = name_en
    OR address_ar IN ('N/A', '')
...
-- osm_node:6037364085 -- https://www.openstreetmap.org/node/6037364085
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'pharmacy' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'Al-Zahra Pharmacy' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'Al-Zahra Pharmacy' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN '' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN '' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN NULL ELSE phone END,
  updated_at = NOW()
...
-- osm_node:6037364086 -- https://www.openstreetmap.org/node/6037364086
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'pharmacy' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'صيدلية جيت' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'صيدلية جيت' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN '' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN '' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN NULL ELSE phone END,
  updated_at = NOW()
WHERE latitude = 32.2158694 AND longitude = 35.1693834
  AND (
    type IS NULL
    OR name_ar = name_en
    OR address_ar IN ('N/A', '')
...
-- osm_node:6771841957 -- https://www.openstreetmap.org/node/6771841957
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'laboratory' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'בית רופאים קדומים' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'בית רופאים קדומים' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN '' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN '' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN NULL ELSE phone END,
  updated_at = NOW()
WHERE latitude = 32.2114265 AND longitude = 35.160584
  AND (
    type IS NULL
    OR name_ar = name_en
    OR address_ar IN ('N/A', '')
...
-- osm_node:6832307686 -- https://www.openstreetmap.org/node/6832307686
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'pharmacy' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'صيدلية الترياق' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'صيدلية الترياق' ELSE name_en END,
  ...
  phone = CASE WHEN phone IS NULL THEN NULL ELSE phone END,
  updated_at = NOW()
WHERE latitude = 32.1766492 AND longitude = 35.3357156
  AND (
    type IS NULL
    OR name_ar = name_en
    OR address_ar IN ('N/A', '')
...
-- osm_node:7302015185 -- https://www.openstreetmap.org/node/7302015185
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'hospital' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'عيادة مخيم الفارعة ( الوكالة)' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'عيادة مخيم الفارعة ( الوكالة)' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN '' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN '' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN NULL ELSE phone END,
  updated_at = NOW()
WHERE latitude = 32.2925402 AND longitude = 35.3454238
  AND (
    type IS NULL
    OR name_ar = name_en
    OR address_ar IN ('N/A', '')
...
-- osm_node:8401981417 -- https://www.openstreetmap.org/node/8401981417
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'pharmacy' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'Al Ayed Pharmacy' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'Al Ayed Pharmacy' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN '' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN '' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN '059-8979013' ELSE phone END,
  updated_at = NOW()
WHERE latitude = 32.2921498 AND longitude = 35.3451928
  AND (
    ...
    OR address_ar IN ('N/A', '')
    OR address_en IN ('N/A', '')
    OR (phone IS NULL AND '059-8979013' IS NOT NULL)
  );

-- osm_node:9887050917 -- https://www.openstreetmap.org/node/9887050917
UPDATE medorbit.clinics SET
  type = CASE WHEN type IS NULL THEN 'clinic' ELSE type END,
  name_ar = CASE WHEN name_ar = name_en THEN 'عيادة ديرشرف' ELSE name_ar END,
  name_en = CASE WHEN name_ar = name_en THEN 'عيادة ديرشرف' ELSE name_en END,
  address_ar = CASE WHEN address_ar IN ('N/A', '') THEN '' ELSE address_ar END,
  address_en = CASE WHEN address_en IN ('N/A', '') THEN '' ELSE address_en END,
  phone = CASE WHEN phone IS NULL THEN NULL ELSE phone END,
  updated_at = NOW()
WHERE latitude = 32.2528069 AND longitude = 35.1901768
  AND (
    type IS NULL
    OR name_ar = name_en
    OR address_ar IN ('N/A', '')
