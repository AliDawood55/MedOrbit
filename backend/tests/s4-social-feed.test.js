...
(async()=>{console.log('\nS4 doctor social feed adversarial tests\n');try{
 ...
 const anonymous=await request('GET','/feed/posts?limit=1',null);
 check('public feed includes eligible post',anonymous.status===200&&anonymous.body.data.items[0]?.id===postId);
 check('anonymous feed omits personalized flags',!('liked_by_me'in anonymous.body.data.items[0])&&!('following_doctor'in anonymous.body.data.items[0]));
 ...
 const page1=await request('GET','/feed/posts?limit=2',null); const page2=await request('GET',`/feed/posts?limit=2&cursor=${encodeURIComponent(page1.body.data.next_cursor)}`,null);
 ...
 const comments=await request('GET',`/feed/posts/${postId}/comments`,null);
 ...
 check('hidden comment excluded',!(await request('GET',`/feed/posts/${postId}/comments`,null)).body.data.some(c=>c.id===commentId));
 ...
 check('hidden post excluded from feed',!(await request('GET','/feed/posts?limit=30',null)).body.data.items.some(p=>p.id===postId));
 ...
 check('admin can restore approved post',(await request('GET','/feed/posts?limit=30',null)).body.data.items.some(p=>p.id===postId));
 await request('POST',`/admin/social/posts/${second.body.data.id}/moderate`,token(admin),{action:'reject'});
 check('rejected post excluded from feed',!(await request('GET','/feed/posts?limit=30',null)).body.data.items.some(p=>p.id===second.body.data.id));

 await pool.query("UPDATE medorbit.doctors SET approval_status='suspended' WHERE id=$1",[doc1.doctorId]);
 check('suspended doctor post excluded from feed',!(await request('GET','/feed/posts?limit=30',null)).body.data.items.some(p=>p.id===postId));
 ...
 check('soft-deleted post excluded from feed',!(await request('GET','/feed/posts?limit=30',null)).body.data.items.some(p=>p.id===third.body.data.id));
