...
void main() {
  testWidgets('shows a loading state while the initial profile is in flight', (tester) async {
  ...
  testWidgets('shows a safe error with retry, and retry reloads the profile', (tester) async {
    final api = _FakeProfileApi()
      ..getMeResults.add(const ApiException(message: 'Network down', code: ApiException.codeServiceUnavailable))
  ...
  testWidgets('editing the English first name and saving sends the updated draft', (tester) async {
    await _useTallSurface(tester);
    final api = _FakeProfileApi()
      ..getMeResults.add(_profile())
    ...
    await tester.tap(find.widgetWithText(ElevatedButton, _strings.saveChangesAction));
    await tester.pump();
    await tester.pump();
  ...
  testWidgets('cancel reverts an in-progress edit without saving', (tester) async {
    await _useTallSurface(tester);
    final api = _FakeProfileApi()..getMeResults.add(_profile());
    await tester.pumpWidget(_app(api: api));
    await tester.pump();
    ...
    await tester.tap(find.widgetWithText(OutlinedButton, _strings.cancelChangesAction));
    await tester.pump();
  ...
  testWidgets('clearing a required name field blocks save with a validation message', (tester) async {
    await _useTallSurface(tester);
    final api = _FakeProfileApi()..getMeResults.add(_profile());
    await tester.pumpWidget(_app(api: api));
    await tester.pump();
    ...
    expect(find.text('${_strings.firstNameEnLabel} is required'), findsOneWidget);
    expect(api.updateMeCalls, isEmpty);
  });

  testWidgets('the language selector switches language and syncs to the server', (tester) async {
    await _useTallSurface(tester);
    final api = _FakeProfileApi()
      ..getMeResults.add(_profile())
  ...
  testWidgets('the theme selector updates the selected segment', (tester) async {
    await _useTallSurface(tester);
    final api = _FakeProfileApi()..getMeResults.add(_profile());
    await tester.pumpWidget(_app(api: api));
    await tester.pump();
  ...
  testWidgets('opens the change-password sheet and validates the confirm field', (tester) async {
    await _useTallSurface(tester);
    final api = _FakeProfileApi()..getMeResults.add(_profile());
    await tester.pumpWidget(_app(api: api));
    await tester.pump();
    ...
    final sheetFields = find.descendant(of: find.byType(ChangePasswordSheet), matching: find.byType(TextFormField));
    ...
    await tester.tap(
      find.descendant(of: find.byType(ChangePasswordSheet), matching: find.widgetWithText(ElevatedButton, _strings.changePasswordAction)),
    );
    await tester.pump();
  ...
  testWidgets('a wrong current password shows a safe inline error in the sheet', (tester) async {
    await _useTallSurface(tester);
    final api = _FakeProfileApi()
      ..getMeResults.add(_profile())
      ..changePasswordResults.add(const ApiException(message: 'Current password is incorrect', code: 'INVALID_CREDENTIALS'));
    await tester.pumpWidget(_app(api: api));
    await tester.pump();
  ...
  testWidgets('shows the initial-letter avatar fallback when there is no avatar_url', (tester) async {
    final api = _FakeProfileApi()..getMeResults.add(_profile(avatarUrl: null));
    await tester.pumpWidget(_app(api: api));
    await tester.pump();
  ...
  testWidgets('tapping change-photo does not crash when the platform picker is unavailable', (tester) async {
    final api = _FakeProfileApi()..getMeResults.add(_profile());
    await tester.pumpWidget(_app(api: api));
    await tester.pump();
  ...
  testWidgets('focusing a field does not throw or leave the save button unreachable', (tester) async {
    await _useTallSurface(tester);
    final api = _FakeProfileApi()..getMeResults.add(_profile());
    await tester.pumpWidget(_app(api: api));
    await tester.pump();
    ...
    expect(tester.takeException(), isNull);
    expect(find.widgetWithText(ElevatedButton, _strings.saveChangesAction), findsOneWidget);
  });
}
  ...
  return ProviderScope(
    ...
    child: MaterialApp(
      ...
      home: MediaQuery(
        ...
        child: Directionality(textDirection: direction, child: const ProfileScreen()),
      ),
    ),
  );
}
...
class _FakeProfileApi extends ProfileApi {
  ...
  @override
  Future<String> uploadAvatar({required String filePath, required String fileName}) {
