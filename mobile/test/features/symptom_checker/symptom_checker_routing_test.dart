...
/// for P6C and found stale: `symptomCheckerApiProvider` reads [dioProvider]
/// — the authenticated backend client. There is no direct-AI-service client
/// in production anymore; this test exercises the real provider graph (no
/// `symptomCheckerApiProvider` override) so a future regression that rewires
/// it off the authenticated backend gateway fails here instead of only
/// showing up as a 401 in the field.
void main() {
  test(
    'symptomCheckerApiProvider posts through the authenticated backend dioProvider',
    () async {
      ...
      await container.read(symptomCheckerApiProvider).checkSymptoms([
        'headache',
      ]);
