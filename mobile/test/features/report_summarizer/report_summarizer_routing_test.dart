...
/// for P6C and found stale: `reportSummarizerApiProvider` already reads
/// [dioProvider] — the authenticated backend client, which is also what lets
/// the backend attribute a summary to the signed-in patient — not
/// [aiDioProvider]. This test exercises the real provider graph (no
/// `reportSummarizerApiProvider` override) so a future regression that
/// rewires it onto [aiDioProvider] fails here instead of only showing up as a
/// 401, or a summary silently missing from My Reports, in the field.
void main() {
  test('reportSummarizerApiProvider posts through dioProvider, never aiDioProvider', () async {
    ...
    await container.read(reportSummarizerApiProvider).summarizeText(text: 'Patient reports mild headache.');
